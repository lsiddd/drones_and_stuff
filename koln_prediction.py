...
def load_data(user):
    with open(user) as u:
        lines = [[float(i.split()[2]) * multiplier, float(i.split()
                                                          [3]) * multiplier] for i in u.readlines()]
...
def create_sequences(user_data, seq_size=20):
    ...
    output = np.empty((len(sequences), seq_size, 2))
...
def create_model(learning_rates, n_layers, n_lstm_cells,
    ...
    model = Sequential()
    model.add(LSTM(n_lstm_cells, input_shape=(9, 2), dropout=droput_rate,
...
def main():
    ...
    for user in users:
        ...
        model.fit(train, target, epochs=20, verbose=1,
                  batch_size=10, validation_split=0.2, callbacks=[])
        model.save("users_koln/{user}.h5")
