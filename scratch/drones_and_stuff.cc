...
std::vector<Ipv4Address> user_ip(numUes);
std::vector<double> user_requests(numUes, 0);
...
void just_a_monitor() {
  ...
  // print metrics peek
  LOG("Simulation time: " << Simulator::Now().GetSeconds());
  LOG("peek_service_level " << peek_service_level.back());
  LOG("peek_uav_usage " << peek_uav_usage.back());
  LOG("peek_cell_throughput " << peek_cell_throughput.back());
  LOG("peek_user_throughput " << peek_user_throughput.back());
