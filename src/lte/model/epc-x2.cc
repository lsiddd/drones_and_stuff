...
void EpcX2::AddX2Interface(uint16_t localCellId, Ipv4Address localX2Address,
                           uint16_t remoteCellId, Ipv4Address remoteX2Address) {
  ...
  m_x2InterfaceCellIds[localX2cSocket] =
      Create<X2CellInfo>(localCellId, remoteCellId);
...
void EpcX2::DoSendHandoverRequest(
  ...
  // Build the X2 packet
  Ptr<Packet> packet =
      (params.rrcContext != 0) ? (params.rrcContext) : (Create<Packet>());
