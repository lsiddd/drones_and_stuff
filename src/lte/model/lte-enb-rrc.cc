...
void UeManager::RecvSnStatusTransfer(
  ...
  NS_LOG_FUNCTION(this);
  for (std::vector<EpcX2Sap::ErabsSubjectToStatusTransferItem>::iterator
           erabIt = params.erabsSubjectToStatusTransferList.begin();
    ...
    // std::map <uint8_t, Ptr<LteDataRadioBearerInfo> >::iterator drbIt =
    // m_drbMap.find (drbId); NS_ASSERT_MSG (drbIt != m_drbMap.end (), "could
    // not find DRBID " << (uint32_t) drbId); drbIt->second->m_pdcp->SetStatus
    // (status);
  }
}
...
void UeManager::RecvRrcConnectionSetupCompleted(
  ...
  case CONNECTION_SETUP:
    ...
    if (m_caSupportConfigured == false &&
        m_rrc->m_numberOfComponentCarriers > 1) {
...
uint16_t UeManager::GetRnti(void) const { return m_rnti; }

uint64_t UeManager::GetImsi(void) const { return m_imsi; }
...
UeManager::State UeManager::GetState(void) const { return m_state; }
...
void UeManager::SwitchToState(State newState) {
  ...
  case CONNECTED_NORMALLY: {
    ...
    if (m_pendingStartDataRadioBearers == true &&
        m_caSupportConfigured == true) {
      StartDataRadioBearers();
    }
  } break;
...
LteEnbRrc::LteEnbRrc()
    : m_x2SapProvider(0), m_cmacSapProvider(0),
      m_handoverManagementSapProvider(0), m_ccmRrcSapProvider(0),
      m_anrSapProvider(0), m_ffrRrcSapProvider(0), m_rrcSapUser(0),
      m_macSapProvider(0), m_s1SapProvider(0), m_cphySapProvider(0),
...
void LteEnbRrc::DoRecvHandoverRequest(EpcX2SapUser::HandoverRequestParams req) {
  ...
  uint16_t rnti = AddUe(UeManager::HANDOVER_JOINING,
                        CellToComponentCarrierId(req.targetCellId));
  ...
  LteEnbCmacSapProvider::RachConfig rc =
      m_cmacSapProvider.at(0)->GetRachConfig();
...
void LteEnbRrc::SetCsgId(uint32_t csgId, bool csgIndication) {
  ...
  for (uint8_t componentCarrierId = 0; componentCarrierId < m_sib1.size();
       componentCarrierId++) {
