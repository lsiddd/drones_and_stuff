...
LteUeRrc::LteUeRrc()
    : m_cmacSapProvider(0), m_rrcSapUser(0), m_macSapProvider(0),
      m_asSapUser(0), m_ccmRrcSapProvider(0), m_state(IDLE_START), m_imsi(0),
      m_rnti(0), m_cellId(0), m_useRlcSm(true), m_connectionPending(false),
      m_hasReceivedMib(false), m_hasReceivedSib1(false),
      m_hasReceivedSib2(false), m_csgWhiteList(0), m_noOfSyncIndications(0),
      m_leaveConnectedMode(false), m_previousCellId(0),
      m_connEstFailCountLimit(0), m_connEstFailCount(0),
...
TypeId LteUeRrc::GetTypeId(void) {
  static TypeId tid =
      TypeId("ns3::LteUeRrc")
          .SetParent<Object>()
          .SetGroupName("Lte")
...
uint64_t LteUeRrc::GetImsi(void) const { return m_imsi; }
...
LteUeRrc::State LteUeRrc::GetState(void) const {
  NS_LOG_FUNCTION(this);
  return m_state;
}
...
void LteUeRrc::DoInitialize(void) {
  NS_LOG_FUNCTION(this);
...
void LteUeRrc::InitializeSap(void) {
...
void LteUeRrc::DoReportUeMeasurements(
  ...
  for (newMeasIt = params.m_ueMeasurementsList.begin();
    ...
    if (params.m_componentCarrierId != 0) {
      ...
      // measurements related to secondary carriers are saved on a different
      // portion of memory
...
void LteUeRrc::DoRecvRrcConnectionReconfiguration(
  ...
  case CONNECTED_NORMALLY:
    if (msg.haveMobilityControlInfo) {
      ...
      m_srb1 = 0; // new instance will be be created within
...
void LteUeRrc::EvaluateCellForSelection() {
  ...
  if (qRxLevMeas - qRxLevMin > 0) {
    ...
    isSuitableCell =
        (cellCsgIndication == false) || (cellCsgId == m_csgWhiteList);
...
void LteUeRrc::ApplyRadioResourceConfigDedicatedSecondaryCarrier(
  ...
  for (std::list<LteRrcSap::SCellToAddMod>::iterator it =
           nonCec.sCellsToAddModList.begin();
...
void LteUeRrc::ApplyRadioResourceConfigDedicated(
  ...
  NS_LOG_FUNCTION(this);
  const struct LteRrcSap::PhysicalConfigDedicated &pcd =
      rrcd.physicalConfigDedicated;
  ...
  std::list<LteRrcSap::SrbToAddMod>::const_iterator stamIt =
      rrcd.srbToAddModList.begin();
...
void LteUeRrc::ApplyMeasConfig(LteRrcSap::MeasConfig mc) {
  ...
  for (std::list<LteRrcSap::ReportConfigToAddMod>::iterator it =
    ...
    std::map<uint8_t, LteRrcSap::ReportConfigToAddMod>::iterator
        reportConfigIt = m_varMeasConfig.reportConfigList.find(reportConfigId);
...
void LteUeRrc::SaveUeMeasurements(uint16_t cellId, double rsrp, double rsrq,
  ...
  NS_LOG_DEBUG(this << " IMSI " << m_imsi << " state " << ToString(m_state)
                    << ", measured cell " << m_cellId << ", new RSRP " << rsrp
                    << " stored " << storedMeasIt->second.rsrp << ", new RSRQ "
                    << rsrq << " stored " << storedMeasIt->second.rsrq);
...
void LteUeRrc::CancelEnteringTrigger(uint8_t measId, uint16_t cellId) {
  ...
  while (it2 != it1->second.end()) {
    ...
    for (it3 = it2->concernedCells.begin(); it3 != it2->concernedCells.end();
         ++it3) {
      if (*it3 == cellId) {
...
void LteUeRrc::SendMeasurementReport(uint8_t measId) {
  ...
  std::map<uint16_t, MeasValues>::iterator servingMeasIt =
      m_storedMeasValues.find(m_cellId);
  ...
  NS_LOG_INFO(this << " reporting serving cell "
                      "RSRP "
                   << (uint32_t)measResults.rsrpResult << " ("
                   << servingMeasIt->second.rsrp
                   << " dBm) "
                      "RSRQ "
                   << (uint32_t)measResults.rsrqResult << " ("
  ...
  if (measReportIt == m_varMeasReportList.end()) {
    ...
    std::map<uint16_t, MeasValues>::iterator sCellsMeasIt =
        m_storedScellMeasValues.begin();
    if (sCellsMeasIt != m_storedScellMeasValues.end()) {
      ...
      for (sCellsMeasIt = m_storedScellMeasValues.begin();
        ...
        measResults.measScellResultList.measResultScell.push_back(
            measResultScell);
      }
    }
...
void LteUeRrc::SaveScellUeMeasurements(uint16_t sCellId, double rsrp,
                                       double rsrq, bool useLayer3Filtering,
                                       uint16_t componentCarrierId) {
  NS_LOG_FUNCTION(this << sCellId << componentCarrierId << rsrp << rsrq
                       << useLayer3Filtering);
  if (sCellId == m_cellId) {
